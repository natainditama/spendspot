...
import { createIcon, PrimitiveIcon, type IPrimitiveIcon, Svg } from "@gluestack-ui/core/icon/creator";
import { Path } from "react-native-svg";
import { styled } from "nativewind";
import { type VariantProps, tva } from "@gluestack-ui/utils/nativewind-utils";

/**
 * Root primitive icon wrapper for SVG rendering across mobile platforms. Wraps
 * base vector elements with scalable dimensions and theme color support.
 */
...
/**
 * Style variant generator controlling icon dimensions and presentation. Defines
 * standard size tokens from 2xs to xl with foreground color mapping.
 */
const iconStyle = tva({
...
/**
 * Universal icon component supporting sizing tokens and custom pixel
 * dimensions. Forwards refs and combines styling classes with accessible SVG
 * rendering.
 */
...
/**
 * Factory helper function producing customized SVG icon components. Binds SVG
 * paths and viewBox definitions to design system icon styles.
 */
...
// All Icons
/**
 * Plus symbol icon typically utilized for creation and addition actions.
 * Renders vector stroke paths conforming to standardized 24x24 viewBox bounds.
 */
...
/**
 * Circular warning icon communicating alerts, errors, or caution notices.
 * Renders an exclamation mark inside a circular border for high visibility.
 */
...
/**
 * Directional arrow icon pointing upwards for scrolling or value increases.
 * Renders upward vector arrow paths within standardized icon dimensions.
 */
...
/**
 * Directional arrow icon pointing downwards for scrolling or descending sorts.
 * Renders downward vector arrow paths within standardized icon dimensions.
 */
...
/**
 * Directional arrow icon pointing towards the right for forward navigation.
 * Renders right-facing vector arrow paths within standardized icon dimensions.
 */
...
/**
 * Directional arrow icon pointing towards the left for backward navigation.
 * Renders left-facing vector arrow paths within standardized icon dimensions.
 */
...
/**
 * At-symbol icon representing email mentions and user handle inputs. Renders
 * standard typography glyph paths conforming to icon boundaries.
 */
...
/**
 * Notification bell icon indicating alerts, updates, or reminder badges.
 * Renders vector chime contours within standardized icon dimensions.
 */
...
/**
 * Calendar sheet icon representing dates, appointments, and scheduling. Renders
 * day grid outlines within standardized icon boundaries.
 */
...
/**
 * Checkmark icon indicating success, completion, or verified selections.
 * Renders vector stroke paths signifying active check states.
 */
...
/**
 * Enclosed checkmark badge signifying successful or confirmed status. Renders a
 * circular border surrounding a centralized checkmark path.
 */
...
/**
 * Compact chevron icon pointing upwards for collapsing expandable content.
 * Renders a directional angle stroke within standardized icon bounds.
 */
...
/**
 * Compact chevron icon pointing downwards for expanding accordion elements.
 * Renders a downward angle stroke within standardized icon bounds.
 */
...
/**
 * Compact chevron icon pointing left for back navigation and pagination.
 * Renders a left-facing angle stroke within standardized icon bounds.
 */
...
/**
 * Compact chevron icon pointing right for drilldown navigation and menus.
 * Renders a right-facing angle stroke within standardized icon bounds.
 */
...
/**
 * Double chevron icon indicating fast reverse pagination or skip to start.
 * Renders two consecutive leftward angles within standard dimensions.
 */
...
/**
 * Double chevron icon indicating fast forward pagination or skip to end.
 * Renders two consecutive rightward angles within standard dimensions.
 */
...
/**
 * Dual vertical chevron icon indicating collapsible menus or sortable tables.
 * Displays opposing vertical chevrons within standard icon boundaries.
 */
...
/**
 * Unfilled or filled circular glyph used for status bullets and radio states.
 * Renders geometric circular paths within standardized icon dimensions.
 */
...
/**
 * Analog clock face icon representing timestamps, schedules, and duration.
 * Renders a circular clock frame with internal hour and minute hands.
 */
...
/**
 * Cross symbol icon used to dismiss dialogs, clear fields, or cancel
 * operations. Renders intersecting diagonal strokes conforming to icon bounds.
 */
...
/**
 * Enclosed cross icon indicating cancellation, errors, or negative status.
 * Renders a circular border encasing a centered cross symbol path.
 */
...
/**
 * Duplicate document icon representing clipboard copying and file duplication.
 * Renders overlapping card outlines within standardized icon dimensions.
 */
...
/**
 * Arrow pointing down into a tray indicating file download and export. Renders
 * downward vector arrows and base container contours.
 */
...
/**
 * Pencil or quill icon representing editing, authoring, and modifications.
 * Renders pen nib strokes within standardized icon boundaries.
 */
...
/**
 * Open eye icon representing visibility, preview actions, and revealed
 * passwords. Renders eye contours with a central pupil path for visibility
 * toggles.
 */
...
/**
 * Slashed eye icon indicating hidden visibility or masked confidential values.
 * Renders eye contours intersected by a diagonal concealing stroke.
 */
...
/**
 * Heart-shaped glyph representing favorites, likes, and bookmarks. Renders
 * curved heart vector contours conforming to icon dimensions.
 */
...
/**
 * Meridian globe icon representing language selection, web, and localization.
 * Renders circular sphere contours with intersecting latitude lines.
 */
...
/**
 * Drag handle icon with vertical dot grid indicating reorderable list items.
 * Renders tactile grip dots for drag-and-drop user interfaces.
 */
...
/**
 * Circular question mark icon indicating context help, tooltips, and FAQs.
 * Renders an informational question glyph centered within a circle.
 */
...
/**
 * Circular information badge indicating helpful hints and contextual notices.
 * Renders an info glyph centered within a circular outline.
 */
...
/**
 * Interlocking chain link icon representing hyperlinks, URLs, and connections.
 * Renders linked chain loops within standardized icon dimensions.
 */
...
/**
 * Diagonal arrow exiting an open box indicating outbound destination links.
 * Renders directional navigation cues for external third-party sites.
 */
...
/**
 * Circular segmented loader icon indicating ongoing background operations.
 * Renders segmented radial notches ready for continuous rotation.
 */
...
/**
 * Closed padlock icon representing privacy, encryption, and secure states.
 * Renders a locked shackle and solid lock body within icon bounds.
 */
...
/**
 * Envelope icon representing email messages, messaging channels, and inboxes.
 * Renders folded envelope contours within standardized icon boundaries.
 */
...
/**
 * Hamburger icon with triple horizontal lines for toggling navigation drawers.
 * Renders stacked parallel bars indicating expandable menu panels.
 */
...
/**
 * Chat bubble icon representing conversation threads, comments, and messaging.
 * Renders rounded speech bubble contours with a tail pointer.
 */
...
/**
 * Crescent moon icon representing night time, sleep modes, and dark themes.
 * Renders curved crescent vector paths conforming to icon boundaries.
 */
...
/**
 * Bent wire paperclip icon representing file attachments and upload actions.
 * Renders looped paperclip wire paths within standardized icon dimensions.
 */
...
/**
 * Telephone handset icon representing calls, dialers, and voice contact.
 * Renders handset contours conforming to standardized icon dimensions.
 */
...
/**
 * Right-pointing triangle icon used for media playback and resume actions.
 * Renders triangular play glyphs within standardized icon bounds.
 */
...
/**
 * Minus or hyphen symbol icon indicating item removal, deletion, or
 * subtraction. Renders a horizontal stroke path conforming to icon dimensions.
 */
...
/**
 * Looping circular arrows icon representing repetition, loops, and recurring
 * tasks. Renders continuous counter-rotating arrow arcs within icon
 * boundaries.
 */
...
/**
 * Looping circular arrows icon with a number one indicating single-track
 * repeats. Renders recurring arrows surrounding a centered numeral one glyph.
 */
...
/**
 * Magnifying glass icon representing search bars, lookup actions, and filters.
 * Renders a circular lens with a diagonal handle stroke.
 */
...
/**
 * Cog or gear icon representing preferences, configuration, and settings menus.
 * Renders cogwheel teeth contours within standardized icon dimensions.
 */
...
/**
 * Branching node icon representing social sharing, exports, and distribution.
 * Renders interconnected sharing nodes conforming to icon boundaries.
 */
...
/**
 * Diagonal slash icon used for pathway dividers and state negation symbols.
 * Renders an inclined separator stroke within standardized icon bounds.
 */
...
/**
 * Five-pointed star icon indicating ratings, bookmarks, and favorite status.
 * Renders geometric star vector contours within standardized dimensions.
 */
...
/**
 * Radiating sun icon representing daytime, brightness, and light theme modes.
 * Renders a circular central sun with surrounding radiant rays.
 */
...
/**
 * Ellipsis icon with three horizontal dots representing overflow action menus.
 * Renders inline action dots indicating additional contextual options.
 */
...
/**
 * Wastebasket icon representing deletion, removal, and purge operations.
 * Renders a trash can body with removable lid outline strokes.
 */
...
/**
 * Open padlock icon representing unlocked access, decrypted states, and public
 * view. Renders an open shackle separated from the lock body.
 */
